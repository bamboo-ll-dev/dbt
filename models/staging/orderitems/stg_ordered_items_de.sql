...
(SELECT ROW_NUMBER() OVER (PARTITION BY id, kind ORDER BY _sdc_batched_at DESC) AS rn, id, order_id, status, kind, amount AS refund_amount, _sdc_extracted_at FROM `leslunes-raw.shopify_de.transactions` WHERE status='success' AND kind='refund') AS A 
WHERE rn=1
