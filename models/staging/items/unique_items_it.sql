...
SELECT * EXCEPT(row_number) FROM 
   (SELECT row_number() OVER (PARTITION BY line_items.value. id, prop.value.name, line_items.value. sku ORDER BY updated_at DESC) AS row_number,
          ...
          CASE WHEN (line_items.value. sku='' OR UPPER(line_items.value. sku) LIKE '%SET' OR UPPER(line_items.value. sku) LIKE '%BUNDLE%' OR UPPER(line_items.value. sku) LIKE '%TRIO' OR UPPER(line_items.value. sku) LIKE '%DUO') AND prop.value.value IS NOT NULL THEN 
...
(SELECT * EXCEPT(row_number) FROM 
   (SELECT row_number() OVER (PARTITION BY line_items.value. id, line_items.value. sku ORDER BY updated_at DESC) AS row_number,
          ...
          SAFE_CAST(line_items.value. quantity AS INT64) AS line_items__quantity,
          line_items.value. title AS line_items__title,
          CASE WHEN UPPER(line_items.value. sku) LIKE '%SET' OR UPPER(line_items.value. sku) LIKE '%BUNDLE%' OR UPPER(line_items.value. sku) LIKE '%TRIO' OR UPPER(line_items.value. sku) LIKE '%DUO' THEN '' ELSE line_items.value. sku END AS line_items__sku,
          line_items.value. variant_title AS line_items__variant_title,
          line_items.value. vendor AS line_items__vendor,
          line_items.value. fulfillment_service AS line_items__fulfillment_service,
          line_items.value. product_id AS line_items__product_id,
    ...
    WHERE (line_items.value. sku='' OR UPPER(line_items.value. sku) LIKE '%SET' OR UPPER(line_items.value. sku) LIKE '%BUNDLE%' OR UPPER(line_items.value. sku) LIKE '%TRIO' OR UPPER(line_items.value. sku) LIKE '%DUO') AND prop.value.value IS NOT NULL) AS B
WHERE row_number = 1) 
