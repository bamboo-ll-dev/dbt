...
FROM `leslunes-prep.orders.unique_orders_fr` as orders
LEFT JOIN (SELECT 
...
FROM `leslunes-prep.items.unique_items_fr`) as items ON CAST(orders.id AS STRING) = items.order__id
LEFT JOIN ( 
#Columns from unique_refunds
SELECT order_id, SUM(refund_amount) AS refund_amount FROM 
(SELECT ROW_NUMBER() OVER (PARTITION BY id, kind ORDER BY _sdc_batched_at DESC) AS rn, id, order_id, status, kind, amount AS refund_amount, _sdc_extracted_at FROM leslunes-raw.shopify_fr.transactions WHERE status='success' AND kind='refund') AS A 
WHERE rn=1
