...
FROM
leslunes-raw.shopify_fr.orders o
...
LEFT JOIN (SELECT 
  ...
  ,	dc.value.code
FROM `leslunes-raw.shopify_fr.orders`, UNNEST(discount_applications) dc
...
(SELECT updated_at, id, payment_gateway_names.value AS payment_gateway_names FROM leslunes-raw.shopify_fr.orders
