...
FROM `leslunes-prep.dbt_orders.unique_orders_fr` as orders
LEFT JOIN (SELECT 
...
FROM `leslunes-prep.dbt_items.unique_items_fr`) as items ON CAST(orders.id AS STRING) = items.order__id
LEFT JOIN ( 
