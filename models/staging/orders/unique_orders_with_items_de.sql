...
FROM `leslunes-prep.orders.unique_orders_de` as orders #`les-lunes-data-269915.import.unique_orders` as orders
LEFT JOIN (SELECT 
...
SAFE_CAST(line_items__price_all AS FLOAT64) AS line_items__price_all,
SAFE_CAST(line_items__total_discount AS FLOAT64) AS line_items__total_discount,
SAFE_CAST(line_items__total_discount_all AS FLOAT64) AS line_items__total_discount_all,
...
FROM `leslunes-prep.items.unique_items_de`) as items ON CAST(orders.id AS STRING) = items.order__id
LEFT JOIN ( 
#Columns from unique_refunds
SELECT order_id, SUM(refund_amount) AS refund_amount, MIN(refunded_at) AS refunded_at FROM 
(SELECT ROW_NUMBER() OVER (PARTITION BY id, kind ORDER BY _sdc_batched_at DESC) AS rn, id, order_id, status, kind, amount AS refund_amount, _sdc_extracted_at, created_at AS refunded_at FROM `leslunes-raw.shopify_de.transactions` WHERE status='success' AND kind='refund') AS A 
WHERE rn=1
GROUP BY order_id) AS refunds ON CAST(orders.id AS STRING) = CAST(refunds.order_id AS STRING)

#WHERE line_items__name != "ll_hash"
