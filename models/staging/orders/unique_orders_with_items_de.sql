...
FROM `leslunes-prep.dbt_orders.unique_orders_de` as orders 
LEFT JOIN (SELECT 
...
FROM `leslunes-prep.dbt_items.unique_items_de`) as items ON CAST(orders.id AS STRING) = items.order__id
LEFT JOIN ( 
